...
//   { messages:[{role,content}], ficha?:{...campos llenados...} } -> { text }
...
3. **Asesoras**: si te preguntan cuánto podrían arrendar o vender, qué conviene reparar antes de publicar, qué documentos pedirle a un arrendatario, cómo se calcula el reajuste en UF o qué revisar antes de comprar, respondes con criterio profesional.
...
export default async (req: Request, context: Context) => {
  ...
  const mensajes = (Array.isArray(body.messages) ? body.messages : []).slice(-14).map((m: any) => ({
